...
import me.drkmatr1984.storageapi.events.BlockBurnStorageEvent;
import me.drkmatr1984.storageapi.events.BlockIgniteStorageEvent;
...
public class BlockFireListener implements Listener{
	...
	public void onBlockBurn(BlockBurnEvent event){
		...
		blocks.add(block);
		BlockBurnStorageEvent storageEvent = new BlockBurnStorageEvent(breakingEntity, sBlocks, blocks);
		Bukkit.getServer().getPluginManager().callEvent(storageEvent);
		if(storageEvent.isCancelBurn()) {
	...
	public void onBlockIgnite(BlockIgniteEvent event) {
		...
		blocks.add(block);
		BlockIgniteStorageEvent storageEvent = new BlockIgniteStorageEvent(breakingEntity, sBlocks, blocks);
		Bukkit.getServer().getPluginManager().callEvent(storageEvent);
		if(storageEvent.isCancelIgnite()) {
			event.setCancelled(true);
		}
	}
